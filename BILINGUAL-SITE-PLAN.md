...
BaseLayout 增加可选 `ogImage` prop，默认使用这张共享图片，并通过 URL helper 输出包含 GitHub Pages base 的绝对地址。如果未来确有中文文字版分享图，可在页面元数据中用 `Bilingual<string>` 覆盖；在此之前不得输出不存在的图片路径。
